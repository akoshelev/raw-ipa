...
/// and step.
pub type StreamKey<O> = (QueryId, O, Gate);
...
/// result in panic.
pub struct StreamCollection<O, S> {
    inner: Arc<Mutex<HashMap<StreamKey<O>, StreamState<S>>>>,
}

impl<O, S> Default for StreamCollection<O, S> {
    fn default() -> Self {
        Self {
...
impl<O, S> Clone for StreamCollection<O, S> {
    fn clone(&self) -> Self {
        Self {
...
impl<O: TransportIdentity, S: Stream> StreamCollection<O, S> {
    ...
    pub fn add_stream(&self, key: StreamKey<O>, stream: S) {
    ...
    pub fn add_waker(&self, key: &StreamKey<O>, waker: &Waker) -> Option<S> {
        let mut streams = self.inner.lock().unwrap();
