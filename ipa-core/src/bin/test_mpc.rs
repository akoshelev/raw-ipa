...
use ipa_core::{
    cli::{
        playbook::{
            make_clients, make_clients_sharded, secure_add, secure_mul, secure_shuffle, validate,
            InputSource,
        },
        Verbosity,
    },
...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    match args.action {
        TestAction::Multiply => {
            let (clients, _) =
                make_clients(args.network.as_deref(), scheme.clone(), args.wait).await;
        ...
        TestAction::ShardedShuffle => {
            ...
            let clients = make_clients_sharded(
                args.network
                    .as_deref()
